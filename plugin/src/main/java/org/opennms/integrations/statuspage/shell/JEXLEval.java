...
package org.opennms.integrations.statuspage.shell;
...
import org.opennms.integrations.statuspage.StatusPageForwarder;

@Command(scope = "opennms-statuspage", name = "eval-jexl", description = "Evaluate a JEXL expression")
@Service
public class JEXLEval implements Action {
    ...
    public Object execute() {
        ...
        for (Alarm alarm : alarmDao.getAlarms()) {
            numAlarmsProcessed++;
            boolean didMatch = StatusPageForwarder.testAlarmAgainstExpression(e, alarm);
            if (didMatch) {
